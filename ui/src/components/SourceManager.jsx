...
const inputCls = "bg-zinc-900 border border-zinc-600 rounded px-2 py-1 text-sm text-zinc-100 focus:outline-none focus:border-emerald-500"
...
const btnSecondary = "px-3 py-1.5 bg-zinc-700 hover:bg-zinc-600 text-zinc-200 text-sm font-medium rounded transition-colors"
const btnDanger = "px-3 py-1.5 bg-red-700 hover:bg-red-600 text-white text-sm font-medium rounded transition-colors"

const typeBadge = (t) => {
  const map = { channel: 'bg-blue-900/60 text-blue-300', playlist: 'bg-purple-900/60 text-purple-300', video: 'bg-zinc-700 text-zinc-300' }
...
function AddSourceForm({ dialect, onAdded }) {
  ...
  return (
    <form onSubmit={submit} className="bg-zinc-800/60 border border-zinc-700 rounded-lg p-4 space-y-3">
      <p className="text-xs font-semibold text-zinc-400 uppercase tracking-wider">Add Source</p>
      ...
      <div className="flex items-center gap-4">
        <label className="text-xs text-zinc-400 whitespace-nowrap">Max videos: {form.max_videos}</label>
        <input
...
function SourceRow({ source, onUpdated, onDeleted }) {
  ...
  return (
    <tr className="border-t border-zinc-700 hover:bg-zinc-800/30 transition-colors">
      <td className="px-3 py-2 text-sm text-zinc-200 max-w-xs">
      ...
      <td className="px-3 py-2 text-sm text-zinc-300">{source.max_videos}</td>
      <td className="px-3 py-2 text-sm text-zinc-400 max-w-xs">
      ...
      <td className="px-3 py-2">
        {confirmDelete ? (
          ...
          <div className="flex gap-2">
            ...
            <button className="px-3 py-1.5 bg-zinc-800 hover:bg-red-900/40 text-zinc-400 hover:text-red-400 text-sm rounded transition-colors" onClick={() => setConfirmDelete(true)}>Delete</button>
          </div>
        )}
      </td>
    </tr>
    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-1 border-b border-zinc-700">
        {DIALECTS.map(d => (
          ...
          >
            {d}
            {sources[d]?.length > 0 && (
              <span className="ml-1.5 text-xs bg-zinc-700 text-zinc-300 px-1.5 py-0.5 rounded-full">{sources[d].length}</span>
            )}
          </button>
        ))}
      </div>
      ...
      <div className="bg-zinc-800 border border-zinc-700 rounded-lg overflow-hidden">
        {dialectSources.length === 0 ? (
          <div className="py-10 text-center text-zinc-500 text-sm">No sources for {activeTab}. Add one below.</div>
        ) : (
          <table className="w-full">
            <thead>
              <tr className="bg-zinc-800/80 text-xs text-zinc-400 uppercase tracking-wider">
