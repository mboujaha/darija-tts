      ...
      <div className="flex items-center gap-1 overflow-x-auto py-2">
        {STAGES.map(s => (
          <div key={s.key} className="flex items-center gap-1 flex-shrink-0">
            <div className="bg-slate-800 border border-slate-700 rounded-lg p-3 w-28 text-center animate-pulse">
              <div className="h-3 bg-slate-700 rounded mb-2" />
              <div className="h-6 bg-slate-700 rounded" />
            </div>
            {s.key !== 'generated' && <span className="text-slate-600 text-lg">›</span>}
          </div>
        ))}
      </div>
    )
        ...
        return (
          <div key={stage.key} className="flex items-center gap-1 flex-shrink-0">
            <Link to={stage.link} className="group block">
              <div className="bg-slate-800 border border-slate-700 group-hover:border-emerald-700/60 rounded-lg p-3 w-28 text-center transition-colors">
                <p className="text-xs text-slate-500 mb-1 truncate">{stage.label}</p>
                <p className={`text-xl font-bold ${val > 0 ? 'text-slate-100' : 'text-slate-600'}`}>
                  {fmtNum(val)}
                </p>
                <p className="text-xs text-slate-600 mt-0.5">{stage.unit}</p>
                {rate != null && (
...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      ...
      {/* GPU */}
      <section>
        <h3 className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-3">GPU</h3>
        {gpu?.available ? (
          ...
          <p className="text-slate-500 text-sm">No GPU detected</p>
        )}
      </section>
        ...
        {jobs.length === 0 ? (
          ...
          <div className="space-y-2">
            {jobs.map(job => (
              <div
                key={job.id}
                className="bg-slate-800 rounded-lg p-3 border border-slate-700 flex items-center gap-4"
              >
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-xs px-1.5 py-0.5 rounded ${
                      ...
                      job.status === 'completed' ? 'bg-slate-700 text-slate-400' :
                  ...
                  {job.message && (
                    <p className="text-xs text-slate-500 mt-0.5 truncate">{job.message}</p>
                  )}
                </div>
