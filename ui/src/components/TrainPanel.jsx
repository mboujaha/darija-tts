  ...
  return (
    <div className="bg-zinc-800 border border-zinc-700 rounded-lg overflow-hidden">
      ...
      {jobLogs.length > 0 && (
        <div
          ref={logRef}
          className="px-4 py-2 max-h-48 overflow-y-auto bg-zinc-900/60 font-mono text-xs space-y-0.5"
        >
          {jobLogs.map((line, i) => (
  ...
  return (
    <div className="bg-zinc-800 border border-zinc-700 rounded-lg overflow-hidden">
      ...
      >
        ...
        <span className="text-sm font-mono text-zinc-300 flex-1 truncate">{run.id}</span>
      ...
      {expanded && (
        <div className="px-4 pb-4 space-y-3 border-t border-zinc-700">
          ...
          <div className="grid grid-cols-4 gap-2 pt-3">
            {[
              ...
              <div key={label} className="text-center">
                ...
                <p className="text-sm font-mono text-zinc-200">{val ?? '—'}</p>
              </div>
            ))}
          </div>
          ...
          {/* Loss charts */}
          {lossHistory === null ? (
            <p className="text-xs text-zinc-500">Loading loss history…</p>
          ) : lossHistory.length === 0 ? (
            <p className="text-xs text-zinc-500">No loss data recorded yet.</p>
          ) : (
            <div className="space-y-2">
              {trainData.length > 1 && (
                <div>
                  <p className="text-xs text-zinc-500 mb-1">Train loss</p>
              ...
              {evalData.length > 1 && (
                <div>
                  <p className="text-xs text-zinc-500 mb-1">Eval loss</p>
                  <div className="bg-zinc-900 rounded p-2">
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-semibold text-zinc-100">Train</h2>
      </div>
      ...
      <div className="bg-zinc-800 border border-zinc-700 rounded-lg p-4 space-y-4">
        {/* Training type */}
        <div>
          <label className="block text-xs font-medium text-zinc-400 mb-1">Training Type</label>
              ...
              <button
                ...
                className={`px-3 py-1.5 text-xs rounded transition-colors ${
                  trainingType === val
                    ...
                    : 'bg-zinc-900 border border-zinc-600 text-zinc-400 hover:text-zinc-200'
                }`}
              >
                {label}
              </button>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-xs font-medium text-zinc-400 mb-1">
              Epochs: <span className="text-zinc-200">{epochs}</span>
            </label>
          ...
          <div>
            <label className="block text-xs font-medium text-zinc-400 mb-1">
              Batch Size: <span className="text-zinc-200">{batchSize}</span>
            </label>
            <div className="flex gap-2 items-center">
              <input
          ...
          <div>
            <label className="block text-xs font-medium text-zinc-400 mb-1">
              Grad Accumulation: <span className="text-zinc-200">{gradAccum}</span>
            </label>
            <div className="flex gap-2 items-center">
              <input
          ...
          {/* Learning rate */}
          <div>
            <label className="block text-xs font-medium text-zinc-400 mb-1">Learning Rate</label>
            <select
              value={effectiveLR}
        ...
        <div>
          <label className="block text-xs font-medium text-zinc-400 mb-1">
            ...
            <span className="text-zinc-600">(leave empty to download XTTS v2)</span>
          </label>
          <input
            type="text"
            ...
            className="w-full bg-zinc-900 border border-zinc-600 rounded px-3 py-2 text-sm text-zinc-100 font-mono placeholder-zinc-600 focus:outline-none focus:border-emerald-500"
          />
          {checkpoints.length > 0 && (
            <div className="mt-1 space-y-0.5">
              <p className="text-xs text-zinc-600">Or select a local checkpoint:</p>
              {checkpoints.map(ck => (
                <button
                  key={ck.path}
                  onClick={() => setBaseCheckpoint(ck.path)}
                  className="block text-xs text-zinc-400 hover:text-emerald-400 font-mono truncate max-w-full"
                >
      ...
      <div className="space-y-3">
        {jobs.length === 0 ? (
          <p className="text-sm text-zinc-500 py-4">No training jobs yet.</p>
        ) : (
          jobs.map(job => (
